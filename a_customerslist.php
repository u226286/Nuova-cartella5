...
// End of modification Customize Navigation/Pager Panel, by Masino Sinaga, May 2, 2012
	...
	function LoadRecordset() {
	    $recordset = new record_set();
		return $recordset->LoadRecordset1($offset = -1, $rowcnt = -1, $this);
	}
...
<thead><!-- Table header -->
    ...
    <?php
        ...
        echo $html6->html06($a_customers, $Language);
    ?>
	</tr>
</thead>
<tbody>
...
<?php if ($a_customers_list->TotalRecs > 0 && ((MS_SELECTABLE_PAGE_SIZES_POSITION=="Left" && $Language->Phrase("dir")!="rtl") || (MS_SELECTABLE_PAGE_SIZES_POSITION=="Left" && $Language->Phrase("dir")=="rtl"))) { ?>
...
foreach ($sRecPerPageList as $a) {
    $thisDisplayRecs = $a;
    ?>
    <option value="<?php if($thisDisplayRecs > 0 ){echo $thisDisplayRecs;}else{echo "ALL";}?>" <?php if(($thisDisplayRecs > 0 && $a_customers_list->DisplayRecs == $thisDisplayRecs) || $a_customers->getRecordsPerPage() == -1){ ?>selected="selected"<?php } ?>><?php if($thisDisplayRecs > 0){echo $thisDisplayRecs;}else{echo $Language->Phrase("AllRecords");}?></option>
<?php } ?>
</select>
</div>
<?php } ?>
		<?php if (MS_PAGINATION_STYLE==1 && !isset($a_customers_list->Pager)) $a_customers_list->Pager = new cNumericPager($a_customers_list->StartRec, $a_customers_list->DisplayRecs, $a_customers_list->TotalRecs, $a_customers_list->RecRange) ?>
				<?php if ((MS_PAGINATION_STYLE==1 && $a_customers_list->Pager->RecordCount > 0) && (($a_customers_list->Pager->PageCount!=1) || ($a_customers_list->Pager->CurrentPage != 1) || (MS_SHOW_PAGENUM_IF_REC_NOT_OVER_PAGESIZE==TRUE)) ) { ?>
				    <?php
                    $html= new cod_html();
				    echo $html->codhtml($a_customers_list, $Language);
				    ?>
        ...
        <?php if (MS_PAGINATION_STYLE==1 && $a_customers_list->Pager->RecordCount > 0) { ?>
            <?php
                $html2= new cod_html();
                echo $html2->codhtml2($Language, $a_customers_list);
            ?>
		<?php } ?>
		...
		<?php if (MS_PAGINATION_STYLE==2 && !isset($a_customers_list->Pager)) $a_customers_list->Pager = new cPrevNextPager($a_customers_list->StartRec, $a_customers_list->DisplayRecs, $a_customers_list->TotalRecs) ?>
				<?php
                $html=new cod_html();
                $html->codhtml($a_customers_list,$Language);
                ?>
        <?php if (MS_PAGINATION_STYLE==2 && $a_customers_list->Pager->RecordCount > 0) { ?>
            <?php
            $html2=new cod_html();
            $html2->codhtml2($a_customers_list,$Language);
            ?>
		<?php } ?>

<?php if ( ($a_customers_list->TotalRecs > 0 && (MS_SELECTABLE_PAGE_SIZES_POSITION=="Right" && $Language->Phrase("dir")!="rtl") || (MS_SELECTABLE_PAGE_SIZES_POSITION=="Right" && $Language->Phrase("dir")=="rtl"))) { ?>
<div class="ewPager"><span>&nbsp;<?php echo $Language->Phrase("RecordsPerPage") ?>&nbsp;</span>
<input type="hidden" name="t" value="a_customers">
<select name="<?php echo EW_TABLE_REC_PER_PAGE ?>" class="form-control input-sm" onchange="this.form.submit();">
...
<?php } ?>
        ...
        <?php if (MS_PAGINATION_STYLE==1 && $a_customers_list->Pager->RecordCount > 0) { ?>
            <?php
            $html2 = new cod_html();
            echo $html2->codhtml2($a_customers_list, $Language);
            ?>
		<?php } ?>
		<?php if (MS_PAGINATION_STYLE==2 && !isset($a_customers_list->Pager)) $a_customers_list->Pager = new cPrevNextPager($a_customers_list->StartRec, $a_customers_list->DisplayRecs, $a_customers_list->TotalRecs) ?>
